...
/*   Updated: 2026/02/25 10:25:42 by ighannam         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_update_spatial_gestures(t_gestures *gest, t_context *context, int x,
		int y)
{
...
void	ft_update_discrete_gestures(t_gestures *gest, t_context *context,
		int key, int *position)
{
	...
	gest->lshift_lmouseclick.handler(&gest->lshift_lmouseclick, context, key,
		position);
	gest->lshift_rmouseclick.handler(&gest->lshift_rmouseclick, context, key,
		position);
	gest->lshift_num_0_9.handler(&gest->lshift_num_0_9, context, key, position);
	gest->lshift_lalt_num_0_9.handler(&gest->lshift_lalt_num_0_9, context, key,
		position);
	gest->lshift_rgb.handler(&gest->lshift_rgb, context, key, position);
	gest->lshift_lalt_rgb.handler(&gest->lshift_lalt_rgb, context, key,
		position);
}
