...
/*   Updated: 2026/01/30 12:14:20 by ighannam         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_include_sphere(t_scene *scene, t_parser_node *content)
{
	...
	sp->center = ft_new_point(ft_atod(ft_strdup(content->splited_args[1][0])),
			ft_atod(ft_strdup(content->splited_args[1][1])),
			ft_atod(ft_strdup(content->splited_args[1][2])));
	sp->radius = ft_atod(ft_strdup(content->splited_args[2][0])) / 2.0;
...
void	ft_include_cylinder(t_scene *scene, t_parser_node *content)
{
	...
	cy->center = ft_new_point(ft_atod(ft_strdup(content->splited_args[1][0])),
			ft_atod(ft_strdup(content->splited_args[1][1])),
			ft_atod(ft_strdup(content->splited_args[1][2])));
	cy->orientation = ft_new_vector_3d(ft_atod(ft_strdup(content->splited_args[2][0])),
			ft_atod(ft_strdup(content->splited_args[2][1])),
			ft_atod(ft_strdup(content->splited_args[2][2])));
	cy->radius = ft_atod(ft_strdup(content->splited_args[3][0])) / 2.0;
	cy->height = ft_atod(ft_strdup(content->splited_args[4][0]));
...
t_material	ft_form_material_cy(t_parser_node *content)
{
	...
	ft_bzero(&material, sizeof(t_material));
	material.albedo = ft_new_vector_3d(ft_atoi(ft_strdup(content->splited_args[5][0])),
			ft_atoi(ft_strdup(content->splited_args[5][1])),
			ft_atoi(ft_strdup(content->splited_args[5][2])));
	if (!content->splited_args[6] || !content->splited_args[7]
		|| !content->splited_args[8] || !content->splited_args[9]
		|| !content->splited_args[10])
		return (material);
	if (content->splited_args[6])
		material.ka = ft_atod(ft_strdup(content->splited_args[6][0]));
	if (content->splited_args[7])
		material.kd = ft_atod(ft_strdup(content->splited_args[7][0]));
	if (content->splited_args[8])
		material.ks = ft_atod(ft_strdup(content->splited_args[8][0]));
	if (content->splited_args[9])
		material.n = ft_atod(ft_strdup(content->splited_args[9][0]));
	if (content->splited_args[10])
		material.kr = ft_atod(ft_strdup(content->splited_args[10][0]));
	return (material);
}
